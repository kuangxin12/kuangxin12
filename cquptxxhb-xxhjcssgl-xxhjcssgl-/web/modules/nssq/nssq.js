define(function(require, exports, module) {
	...
	var nssqSave = require('./nssqSave');
	var nssqJl = require('./nssqJl');
	var nssqwns = require('./nssqwns');
	var nssqView = require('./nssqView');

	 var taskType = "ALL";//任务类型
	  var flowStatus = "1";//流程类型
	  
	var viewConfig = {
		initialize : function() {
			var view = utils.loadCompiledPage('nssq');
			this.$rootElement.html(view.render({}), true);
			this.pushSubView([ nssqSave ]);
			this.initView();
			...
			//	"[data-action=nsjl]" : this.actionNsjl,
		     ...
		     //   "[data-action=wnsquery]" : this.wnsQuery,
...
//			setTimeout(actionGetShData,20000);
		...
		actionGetShData:function(e){
		  ...
		  actionAdd: function(){
		      var nssqNewTpl = utils.loadCompiledPage('nssqSave');
		      ...
		      //查询当前登录人员在说明字典中的信息
		      var cxtksj = WIS_EMAP_SERV.getData(bs.api.pageModel, 'T_XXB_XXHJCSS_CZSM_QUERY', {SFRZH : USERID,SSYM:'nssq',pageNumber : 1,});  
		      if(cxtksj.rows.length == 0 ){ //如果没有信息，就添加一行
		    	  ...
		    	  BH_UTILS.doAjax('../modules/nssq/T_XXB_XXHJCSS_CZSM_ADD.do', addsj);  
		    	  BH_UTILS //接着打开服务说明
					.bhWindow(
							...
							+"<div id='ampDetailIntroductionContent' class='amp-detail-content'>"
								+"<p><span style='font-family: 微软雅黑, &quot;Microsoft YaHei&quot;;font-size: 18px;'>有效期前一个月可以进行年审申请，申请经申请单位信息化联络员、申请单位负责人、信息化办审核，审核通过后延长对应信息化基础设施的有效期。</span></p>"
								+"<p><span style='font-family: 微软雅黑, &quot;Microsoft YaHei&quot;; font-size: 18px;'>年审申请审核流程如下图：</span></p>"
								+"<div style='margin:0 auto;width:80%'><img src='./public/images/nssqflow.png'"
							 	+" title='年审申请流程图' alt='nssqflow.pn' width='600px' height='130px' style='width: 630px; height:125px;'></div>"
								+"<p><span style='font-family: 微软雅黑, &quot;Microsoft YaHei&quot;; font-size: 18px;'>部分操作说明：</span></p>"
								+"<p><span style='font-family: 微软雅黑, &quot;Microsoft YaHei&quot;; font-size: 14.5px;'>&nbsp&nbsp&nbsp&nbsp新增申请：可以申请所需要年审的信息化基础设施。<span style='color:red'></span></span></p>"
								...
								+"<p><span style='font-family: 微软雅黑, &quot;Microsoft YaHei&quot;; font-size: 14.5px;'>&nbsp&nbsp&nbsp&nbsp流程状态：以查看当前记录的申请与审核记录以及审核意见。</span></p>"
								+"<p><span style='font-family: 微软雅黑, &quot;Microsoft YaHei&quot;; font-size: 14.5px;'>&nbsp&nbsp&nbsp&nbsp撤回：可撤回已提交但信息化联络员未审核的申请记录。</span></p>"
							...
							[
									{
										...
										callback : function() {
											...
											if(sftx){//如果勾选了不再弹出，就把SFTK字段改为false
												 var tksj = WIS_EMAP_SERV.getData(bs.api.pageModel, 'T_XXB_XXHJCSS_CZSM_QUERY', {SFRZH : USERID,SSYM:'nssq'}); 
												 var xgsj ={
														 WID: tksj.rows[0].WID,									    	    
											    	     SFTK:'false'
											    	  }
											    	  BH_UTILS.doAjax('../modules/nssq/T_XXB_XXHJCSS_CZSM_MODIFY.do', xgsj);
											}
											$.bhPaperPileDialog.show({
										        content: nssqNewTpl.render({}),
										        title: "年审申请",
										        ready: function($header, $body, $footer){
										          nssqSave.initialize(null,null,null,null,null);
										        }
										      });
										}
									}, {
		    	...
		    	//  var zccxtksj = WIS_EMAP_SERV.getData(bs.api.pageModel, 'T_XXB_XXHJCSS_CZSM_QUERY', {SFRZH : USERID,SSYM:'nssq'}); 
		  					...
		  					[
		  							{
		  								...
		  								callback : function() {
		  									...
		  									if(sftx){//如果勾选了不再弹出，就把SFTK字段改为false
		  										 var tksj = WIS_EMAP_SERV.getData(bs.api.pageModel, 'T_XXB_XXHJCSS_CZSM_QUERY', {SFRZH : USERID,SSYM:'nssq',pageNumber : 1,}); 
		  										 var xgsj ={
		  												 WID: tksj.rows[0].WID,									    	    
		  									    	     SFTK:'false'
		  									    	  }
		...
		actionDetail : function(e) {
			...
			var nssqViewTpl = utils.loadCompiledPage('nssqSave');
			...
			$.bhPaperPileDialog.show({
				content : nssqViewTpl.render({}),
				title : "查看",
				ready : function($header, $body, $footer) {
					nssqView.initialize(data.rows[0],id,taskid,proid);
//					$("#emapForm").emapForm("setValue", data.rows[0]);
		...
		actionEdit : function(e) {
			...
			var nssqEditTpl = utils.loadCompiledPage('nssqSave');
			...
			$.bhPaperPileDialog.show({
				...
				title : "编辑",
				ready : function($header, $body, $footer) {
					nssqSave.initialize(data.rows[0],id,taskid,jcssid,flowstatus); 
...
//						  _sjtj.nssqtj();
		...
		/*wnsQuery : function(event) {
			  var cxygq = [
			       //{"name":"YXQ","value":"2019-10-16 14:36:00","linkOpt":"AND","builder":"moreEqual"},
			       {"name":"YXQ","caption":"有效期","linkOpt":"AND","builderList":"cbl_Other","builder":"moreEqual","value":"2019-09-01 23:55:55"}
			     ]		        
			  cxygq =  JSON.stringify(cxygq)
			  var cxwgq = [
			       //{"name":"YXQ","value":"2019-10-16 14:36:00","linkOpt":"AND","builder":"moreEqual"},
			       {"name":"YXQ","caption":"有效期","linkOpt":"AND","builderList":"cbl_Other","builder":"more","value":"2019-09-01 23:55:55"}
			     ]		        
			  cxwgq =  JSON.stringify(cxwgq)
			       
			var searchData = $('#emapAdvancedQuery').emapAdvancedQuery('getValue');
			var data = event.currentTarget.attributes.queryType.value;
		    var nszt = data;
		           if(nszt == ""){
		               $('#wnsemapdatatable').emapdatatable('reload', {
		                    querySetting : searchData
		                });
		           }else if(nszt == "ygq"){
		                $('#wnsemapdatatable').emapdatatable('reload', {
		...
		actionLczt:function(e){
	  		var id = $(e.target).attr("data-x-wid");
	    	var pid=_util.getpromid(id,'nssq','T_XXB_XXHJCSS_NS_QUERY');
										...
										var zzhj = Object.getOwnPropertyNames(last)[0]  //最后环节
									      ...
									      if (rowData.FLOWSTATUS == "6" || rowData.FLOWSTATUS == ""|| rowData.FLOWSTATUS == "4"|| rowData.FLOWSTATUS == "2")
									         ...
									         str += '<a href="javascript:void(0)" data-action="edit" data-x-wid=' + rowData.WID + 
									                ...
									                ' data-x-jcssid=' + rowData.WLJCSSWID+ 
									      ...
									      if (rowData.FLOWSTATUS == "1" && dqhj == "xxhllyns") //当流程状态为“审核中”
...
//								             ' data-x-taskid=' + rowData.TASKID+ 
//								             ' data-x-proid=' + rowData.PROCESSINSTANCEID + '>' + '流程状态' + '</a>';
//						            }
//								      return str;
//					            }
//					          }
