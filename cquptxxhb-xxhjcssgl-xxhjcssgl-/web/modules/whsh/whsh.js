define(function(require, exports, module) {
  ...
  var bs = require('./whshBS');
  var whshSave = require('./whshSave');
  var whshView = require('./whshView');
 ...
 // var taskType = "ALL_TASK"; //任务类型
  ...
  var viewConfig = {
    initialize: function() {
      var view = utils.loadCompiledPage('whsh');
      this.$rootElement.html(view.render({}), true);
      this.pushSubView([whshSave]);
      this.initView();
...
//      setTimeout(_sjtj.whshtj,20000);
    ...
    actionAudit:function(e){
    	...
    	var whshEditTpl = utils.loadCompiledPage('whshSave');
        ...
        $.bhPaperPileDialog.show({
          content: whshEditTpl.render({}),
          title: "审核",
          ready: function($header, $body, $footer){
            whshSave.initialize(data,id,taskid,lx,cksq);       
    ...
    actionDetail: function(e){
      var id = $(e.target).attr("data-x-wid");
      var lcxx=_util.getpromid(id,'whsh','xxhbwhlb');
      if(lcxx=='error'){
  		$.bhTip({
				content : '获取流程id失败',
				state : 'danger',
      ...
      var whshViewTpl = utils.loadCompiledPage('whshSave');
      var data = WIS_EMAP_SERV.getData(bs.api.pageModel, 'whssjbxx', {WID_1:id,pageNumber:1});
      var lx = data.rows[0].LX
      var cksq = data.rows[0].CK_SFYSQCK
      ...
      $.bhPaperPileDialog.show({
        content: whshViewTpl.render({}),
        title: "申请详情",
        ready: function($header, $body, $footer){
          whshView.initialize(id,taskid,lx,cksq);
      	 $("#emapForm").emapForm("setValue", data.rows[0]);
        }
      });
    },
  			...
  			success:function(response) {
				...
				if(pd.succeed){
					 var ztparams = {
				    		 ...
				    		 $.ajax({
				    			url:'../modules/whsh/T_XXB_XXHJCSS_JBXX_MODIFY.do',  //修改更新
				    			type:'post',
				    			data:ztparams,
				    			async:false,
				    			cache : true,
				    		 ...
				    		 $.ajax({
 				    			url:'../modules/whsh/T_XXB_XXHJCSS_WH_MODIFY.do',  //修改更新
 				    			type:'post',
 				    			data:{
 				    				WID : id,
		    					    ZT : null
...
//					  _sjtj.whshtj();
  ...
  //流程状态事件
	actionLczt:function(e){
  		var id = $(e.target).attr("data-x-wid");
    	var pid=_util.getpromid(id,'whsh','xxhbwhlb');
    ...
    _initTable: function() {
    	var params = {
            ...
            nodeId : 'whsh', // 流程定义中人工环节的编号，必填
            appName : 'xxhjcssgl',// 应用的名称，必填
            module : 'modules', // 模块名，可以没有，默认modules
            page : 'whsh', // 回调动作的epg的编号，必填
           // action : 'T_XXB_XXHJCSS_WH_QUERY', // 回调的动作，必填
            action : 'xxhbwhlb',
            hideFlowState:true,
			hideTaskState:true,
			querySetting:[{"name":"JHWHSJ","caption":"计划维护时间","builder":"lessEqual","linkOpt":"AND","builderList":"cbl_Other","value":now_time},{"name":"JHWHSJ","caption":"计划维护时间","linkOpt":"AND","builderList":"cbl_String","builder":"moreEqual","value":old_time}],
			'*order': '-JHWHSJ',
        };
        ...
        customColumns: [{
          ...
          column: {
            ...
            if (rowData.PROCESSINSTANCEID != null ){  //流程信息是否为空 	            	
				if (rowData.FLOWSTATUS == "1" && rowData.TASKSTATUS == "1")
