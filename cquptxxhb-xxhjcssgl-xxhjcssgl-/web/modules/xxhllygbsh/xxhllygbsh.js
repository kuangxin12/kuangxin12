define(function(require, exports, module) {
  ...
  var xxhllygbshSave = require('./xxhllygbshSave');
  var xxhllygbshView = require('./xxhllygbshView');
 ...
 // var taskType = "ALL_TASK"; //任务类型
  ...
  var viewConfig = {
    initialize: function() {
      var view = utils.loadCompiledPage('xxhllygbsh');
      this.$rootElement.html(view.render({}), true);
      this.pushSubView([xxhllygbshSave]);
      this.initView();
...
//      setTimeout(actionGetShData,20000);
    ...
    actionAudit:function(e){
    	...
    	var xxhllygbshEditTpl = utils.loadCompiledPage('xxhllygbshSave');
        ...
        $.bhPaperPileDialog.show({
          content: xxhllygbshEditTpl.render({}),
          title: "审核",
          ready: function($header, $body, $footer){
        	  xxhllygbshSave.initialize(data,id,taskid,lx,cksq);       
    ...
    actionDetail: function(e){
      var id = $(e.target).attr("data-x-wid");
      var lcxx=_util.getpromid(id,'xxhllygbsh','T_XXB_XXHJCSS_GB_QUERY');
      if(lcxx=='error'){
  		$.bhTip({
				content : '获取流程id失败',
				state : 'danger',
      ...
      var xxhllygbshViewTpl = utils.loadCompiledPage('xxhllygbshSave');
      var data = WIS_EMAP_SERV.getData(bs.api.pageModel, 'gbssjbxx', {WID_1:id,pageNumber:1});
      var lx = data.rows[0].LX
      var cksq = data.rows[0].CK_SFYSQCK  
      ...
      $.bhPaperPileDialog.show({
        content: xxhllygbshViewTpl.render({}),
        title: "详情",
        ready: function($header, $body, $footer){
          xxhllygbshView.initialize(id,taskid,lx,cksq);
       	 $("#emapForm").emapForm("setValue", data.rows[0]);
        }
      });
    },
...
//					  _sjtj.llygbshtj();			
 ...
 //流程状态事件
	actionLczt:function(e){
 		var id = $(e.target).attr("data-x-wid");
   	var pid=_util.getpromid(id,'xxhllygbsh','T_XXB_XXHJCSS_GB_QUERY');
   ...
   _initTable: function() {
   	...
   	var params = {
           	  ...
           	  nodeId : 'xxhllyshgb', // 流程定义中人工环节的编号，必填
           	  appName : 'xxhjcssgl',// 应用的名称，必填
           	  module : 'modules', // 模块名，可以没有，默认modules
           	  page : 'xxhllygbsh', // 回调动作的epg的编号，必填
           ...
           cellsRenderer: function(row, column, value, rowData) {
           	...
           	if (rowData.PROCESSINSTANCEID != null ){  //流程信息是否为空
				...
				str += '<a href="javascript:void(0)" data-action="audit" data-x-wid='+ rowData.WID
	        	 ...
	        	 if(zzhj == 'xxhllyshgb'&& rowData.FLOWSTATUS == "5"){
...
//					+ '>'+ '详情' + '</a>';
//	        	if (rowData.PROCESSINSTANCEID != null ){  //流程信息是否为空 
//	        	 str += ' | <a href="'+WIS_EMAP_SERV.getContextPath()+'/sys/emapflow/tasks/queryFlowState.do?taskId='+rowData.TASKID+'&defKey=xxhjcssgl.xxhjcssgb" target="_blank"   data-x-wid=' + rowData.WID + 
//	             ' data-x-taskid=' + rowData.TASKID+ 
