define(function(require, exports, module) {
	...
	var xxhllyshsqSave = require('./xxhllyshsqSave');
	var xxhllyshsqView = require('./xxhllyshsqView');
  ...
  setTimeout(window.tabTodoUpdate)
	var viewConfig = {
		initialize : function() {
			self = this;	
			var view = utils.loadCompiledPage('xxhllyshsq');
			this.$rootElement.html(view.render({}), true);
			this.pushSubView([ xxhllyshsqSave ]);
			this.initView();
...
//			setTimeout(actionGetShData,20000);
		...
		actionAudit : function(e) {
			...
			var xxhllyshsqEditTpl = utils.loadCompiledPage('xxhllyshsqSave');
						...
						[
								{
									...
									callback : function() {
										$.bhPaperPileDialog.show({
											content : xxhllyshsqEditTpl.render({}),
											title : sqlx + "审核",
											ready : function($header, $body, $footer) {
												xxhllyshsqSave.initialize(id, data.rows[0],taskid, lx,'xxhllyshsq',sqlx);
...
//   					$("#emapForm").emapForm("setValue", data.rows[0]);
		...
		actionDetail : function(e) {
			var id = $(e.target).attr("data-x-wid");
	    	 var lx = $(e.target).attr("data-x-lx");
	    	 var lcxx=_util.getpromid(id,'xxhllyshsq','T_XXB_XXHJCSS_JBXX_QUERY');
		        if(lcxx=='error'){
		    		$.bhTip({
						content : '获取流程id失败',
						state : 'danger',
	         ...
	         var xxhllyshsqViewTpl = utils.loadCompiledPage('xxhllyshsqSave');
	         ...
	         $.bhPaperPileDialog.show({
	             content : xxhllyshsqViewTpl.render({}),
	             title : "查看",
	             ready : function($header, $body, $footer) {
	            	 xxhllyshsqView.initialize(id,data.rows[0], taskid,lx,'xxhllyshsq');
		  		...
		  		$.ajax({
		  			...
		  			success:function(response) {
						...
						if(pd.succeed){
		    				...
		    				$.ajax({
		    					url:'../modules/xxhllyshsq/T_XXB_XXHJCSS_JBXX_MODIFY.do',  //修改更新
		    					type:'post',
		    					data:ztparams,
		    					async:false,
		    					cache : true,
...
//					        _sjtj.llysqshtj();
		...
		actionLczt:function(e){
	  		var id = $(e.target).attr("data-x-wid");
	    	var pid=_util.getpromid(id,'xxhllyshsq','T_XXB_XXHJCSS_JBXX_QUERY');
					                 ...
					                 },
					                 {
					                	 ...
					                	 column : {
					                		 ...
					                		 cellsRenderer : function(row, column, value,
					                			...
					                			/* 
					                			 var str = '<a href="javascript:void(0)" data-action="detail" data-x-wid='
					             				...
					             				if (rowData.PROCESSINSTANCEID != null ){  //流程信息是否为空
													...
													var zzhj = Object.getOwnPropertyNames(last)[0]  //最后环节			  	       
					                				 ...
					                				 str += ' <a href="javascript:void(0)" data-action="audit" data-x-wid='
					                					 + rowData.WID+' data-x-lx=' + rowData.LX + ' data-x-sqlx=' + rowData.SQLX + ' data-x-taskid='
					                					 + rowData.TASKID+ ' data-x-proid='+ rowData.PROCESSINSTANCEID+ '>审核</a> | ';
					                			 ...
					                			 if(dqhj == 'bmldshsq'){ 
						         	        	 ...
						         	        	 if(zzhj == 'xxhllyshsq'&&rowData.FLOWSTATUS == "5"){
