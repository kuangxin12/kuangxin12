define(function(require, exports, module) {
    ...
    var viewConfig = {
        initialize: function(id,taskid,jcssid,gbsj,flowstatus) {
        	var mode = WIS_EMAP_SERV.getModel(bs.api.pageModel, 'gbjcsssq', 'form');
            $("#emapForm").emapForm({
            ...
            //提醒确认设置字体样式
    	      $("#emapForm [data-caption=提醒确认]").find('label').css({
            ...
            var dataAdapter = new $.jqx.dataAdapter(jcssxx.rows);         
            	...
            	if( id != null ){ //编辑
            		...
            		$("#emapForm [data-name= GBJCSS]").jqxDropDownList('selectItem',
        					jcssdrop.rows[0].PWID);
            		$('#emapForm [data-name=GBSJ]').jqxDateTimeInput({ value: gbsj });
            	}
            	//单选卡片
          	...
          	//单选卡片---end
          	 $("#tsjcss").html("请选择需关闭的信息化基础设施:")  
          	  $(".bh-txt-input__foot").hide();
          	 
          	     $("#emapForm [data-name=GBJCSS]").parent().parent()
 			.parent().css("display", "none");
            this.removeRequired("GBJCSS");
            	//$("#flow").css("display", "none");
            //	$("#lczt").css("display", "none");
            	$("#flow").css("display", "none");
            	var yysj;
   		    	$.ajax({
  	                 type: "post",
  	        ...
  	        //时间设置
  	        if(id == null){
  	     	  $('#emapForm [data-name=GBSJ]').jqxDateTimeInput({ value: null });
  	     	  }		
            var myDate = new Date;
		     ...
		     var dqdate = year + '-' + mon + '-' + date;//当前日期	 
  	        ...
  	        $("#emapForm [data-name=GBSJ]").jqxDateTimeInput({ 
  	        	min: dqdate,
  	        	culture: 'zh-CN',
  	      ...
  	      if(id!=null){  //当编辑不为草稿状态时，暂时不能修改已选择的基础设施
	        	if(flowstatus != 4){
	        	//	$("#emapForm [data-name= WHJCSS]").attr("disabled",true)
	        		$("#emapForm [data-name= GBJCSS]").jqxDropDownList({ disabled: true }); 
            		...
            		/*	  console.log("save js---> type="+type);*/
            			if(type.id == "processStatus") {
              	    	  ...
              	    	  if( $("#emapForm").emapValidate('validate')){
              	    		 if($("#emapForm [data-name=GBSJ]").jqxDateTimeInput('value') == null){
              	    			 $.bhTip({
										content : '请选择关闭时间！',
										state : 'warning',
										hideWaitTime : 2000
              	    		   ...
              	    		   var  xzdate = $("#emapForm [data-name=GBSJ]").jqxDateTimeInput('value');//选择日期
	                  		     var xzyear = xzdate.getFullYear(); //获取选择年
		                   		 var xzmon = xzdate.getMonth() + 1; //获取选择月
		                   		 var xzdate = xzdate.getDate(); //获取选择日
		                   		var  gbrq = xzyear + '-' + xzmon + '-' + xzdate;//选择日期	
	                  		 
                  		
                  		    	 
	                  		    //var jcsswid = $("#emapForm [data-name=GBJCSS]").val()
		                   		var jcsswid= "";
										 ...
										 buttons:[
										      ...
										      callback:function(){
										    	 /* var candidateArray=_funauth.newqueryXxhllyCandidate();
										    	  ...
										    	  var candidateArray=_funauth.queryXxhfgldSimple('信息化联络员',bghglyDw,'xxhjcssgbsq');
					  								if(candidateArray=='NoCandidate'||candidateArray==undefined||candidateArray==''){
					  									$.bhTip({
					  										content : _funauth.noxxhlly,
					  										state : 'warn',
				 	            	    		 ...
				 	            	    		 var ztparams = {
				 	    				    		 ...
				 	    				    		 $.ajax({
				 	    				    			url:'../modules/xxhjcssgbsq/T_XXB_XXHJCSS_JBXX_MODIFY.do',  //修改更新
				 	    				    			type:'post',
				 	    				    			data:ztparams,
				 	    				    			async:false,
				 	    				    			cache : true,
