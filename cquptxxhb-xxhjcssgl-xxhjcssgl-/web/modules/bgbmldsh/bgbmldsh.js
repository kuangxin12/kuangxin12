define(function(require, exports, module) {
  ...
  var bgbmldshSave = require('./bgbmldshSave');
  var bgbmldshView = require('./bgbmldshView');
 ...
 // var taskType = "ALL_TASK"; //任务类型
  ...
  var viewConfig = {
    initialize: function() {
      var view = utils.loadCompiledPage('bgbmldsh');
      this.$rootElement.html(view.render({}), true);
      this.pushSubView([bgbmldshSave]);
      this.initView();
...
//      setTimeout(actionGetShData,20000);
    ...
    /*actionAudit: function(e){
        ...
        var bgbmldshEditTpl = utils.loadCompiledPage('bgbmldshSave');
            
        $.bhPaperPileDialog.show({
          content: bgbmldshEditTpl.render({}),
          title: "审核",
          ready: function($header, $body, $footer){
            bgbmldshSave.initialize(data,id,taskid,lx,cksq);       
    ...
    actionAudit: function(e){
      ...
      $.bhPaperPileDialog.show({
        ...
        title: "审核",
        ready: function($header, $body, $footer){
          bgbmldshSave.initialize(data,id,taskid);       
          $("#emapForm").emapForm("setValue", data.rows[0]);     
        }
      });
    },
   ...
   /* actionDetail: function(e){
     ...
     var lx = data.rows[0].LX
     var cksq = data.rows[0].CK_SFYSQCK
      var bgbmldshViewTpl = utils.loadCompiledPage('bgbmldshSave');     
     if(data.rows[0].CK_SFYSQCK=='是'){
      ...
      $.bhPaperPileDialog.show({
        content: bgbmldshViewTpl.render({}),
        title: "申请详情",
        ready: function($header, $body, $footer){
          bgbmldshView.initialize(id,taskid,lx,cksq);
     	 $("#emapForm").emapForm("setValue", data.rows[0]);
        }
      });
    },*/
    actionDetail: function(e){
        var id = $(e.target).attr("data-x-wid");
        var lcxx=_util.getpromid(id,'bgbmldsh','T_XXB_XXHJCSS_ZRRBGJL_QUERY');
        if(lcxx=='error'){
    		$.bhTip({
				content : '获取流程id失败',
				state : 'danger',
      ...
      var data = WIS_EMAP_SERV.getData(bs.api.pageModel, 'T_XXB_XXHJCSS_ZRRBGJL_QUERY', {WID:id,pageNumber:1});
        ...
        $.bhPaperPileDialog.show({
          ...
          title: "申请详情",
          ready: function($header, $body, $footer){
            bgbmldshView.initialize(data,id);
       	 $("#emapForm").emapForm("setValue", data.rows[0]);
          }
        });
      },
...
//					  _sjtj.fzrbgshtj();
   ...
   //流程状态事件
 	actionLczt:function(e){
   		var id = $(e.target).attr("data-x-wid");
     	var pid=_util.getpromid(id,'bgbmldsh','T_XXB_XXHJCSS_ZRRBGJL_QUERY');
             ...
             cellsRenderer: function(row, column, value, rowData) {        
             	...
             	if (rowData.PROCESSINSTANCEID != null ){  //流程信息是否为空
 					...
 					str += ' <a href="javascript:void(0)" data-action="audit" data-x-wid='+ rowData.WID
 		        	 ...
 		        	 if(zzhj == 'bmldshbg'&& rowData.FLOWSTATUS == "5"){
